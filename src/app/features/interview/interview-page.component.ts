...
export default class InterviewPageComponent implements OnInit {
  ...
  openAddType(): void {
    this.dialog
      ...
      .subscribe((name: string | undefined) => {
        if (name != null && name.trim()) this.interviewService.addType(name.trim());
      });
  }
  ...
  openEditType(type: QuestionType): void {
    this.dialog
      ...
      .afterClosed()
      .subscribe((name: string | undefined) => {
        if (name != null && name.trim()) this.interviewService.updateType(type.id, name.trim());
      });
  }

  deleteType(type: QuestionType): void {
    if (confirm(`Delete "${type.name}" and all its questions?`)) {
      this.interviewService.deleteType(type.id);
    }
  }
    ...
    this.dialog
      ...
      .subscribe((q: Partial<Question> | undefined) => {
        if (q?.text != null && q.answer != null)
          this.interviewService.addQuestion({ typeId, text: q.text, answer: q.answer, imageUrls: q.imageUrls });
      });
  }
  ...
  deleteQuestion(question: Question): void {
    if (confirm('Delete this question?')) this.interviewService.deleteQuestion(question.id);
  }
